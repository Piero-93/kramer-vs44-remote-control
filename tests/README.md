...
| `test_protocol_offline.py` | no | Frame generation against the byte sequences confirmed in the manual and on hardware — including the two that differ only in the OUTPUT field, STORE and DELETE PRESET — reply decoding, the `expect` shortcut, end-of-stream handling, the separation of unsolicited frames from replies, and the shared liveness policy, driven through a fake socket |
